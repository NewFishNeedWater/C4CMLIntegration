...
public class C4CControllerForML {
	...
	public C4CUserActionResponse getAIProcessResult(
		...
		try {
			...
			NaturalLanProcessResponse nlpResponse = naturalLanProcessService
					.getNLResponse(request);
			// convert the NLP response to deep learning API request
			C4CUserActionVo deepLearnRequest = mlService
					.convertNLPToUserActionRequest(nlpResponse);
			// get response from deep learning API
			C4CUserActionResponse response = deepLearningService
					.generateResultEntry(deepLearnRequest);
